      ...
      <table>
        ...
        <tbody>
          <tr v-for="c in displayedCases" :key="c.id" class="clickable-row" @click="goToCase(c.id)">
            ...
            <td>{{ c.on_hold_since ? formatDate(c.on_hold_since) : '—' }}</td>
            <td class="reason-cell">{{ c.on_hold_reason || '—' }}</td>
          </tr>
          <tr v-if="!displayedCases.length">
            <td colspan="6" class="empty">No on hold cases</td>
          </tr>
        </tbody>
      </table>
    </div>
...
const fetchCases = async () => {
  try {
    ...
    console.error('Failed to fetch on-hold cases:', err);
  }
};
