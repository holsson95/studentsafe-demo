<template>
  <div class="dashboard-grid">
    ...
    <RecentCasesTable class="recent-cases-area" :case-type-id="caseTypeId" :school-id="schoolId" />
    <div class="card top-categories">
      <TopCategories :case-type-id="caseTypeId" :school-id="schoolId" />
    </div>

    <div class="card cases-over-time">
      <CasesOverTimeChart :case-type-id="caseTypeId" :school-id="schoolId" />
    </div>
    <div class="card overall-chart">
      <OverallCaseChart :case-type-id="caseTypeId" :school-id="schoolId" />
    </div>
    <div class="card cases-by-chart">
      <CasesByTabbedChart :case-type-id="caseTypeId" :school-id="schoolId" />
    </div>
...
import RecentCasesTable from './RecentCasesTable.vue'
import TopCategories from '../charts/TopCategories.vue'
import CasesOverTimeChart from '../charts/CasesOverTimeChart.vue'
import OverallCaseChart from '../charts/OverallCaseChart.vue'
import CasesByTabbedChart from './CasesByTabbedChart.vue'
...
.dashboard-grid {
  ...
  grid-template-rows: auto minmax(0, 1.15fr) minmax(0, 0.85fr) auto;
  grid-template-areas:
    "stats-row stats-row stats-row stats-row"
    "recent-cases recent-cases recent-cases top-categories"
    "cases-over-time cases-over-time overall-chart cases-by-chart"
...
.cases-over-time {
  grid-area: cases-over-time;
}

.overall-chart {
  grid-area: overall-chart;
}

.cases-by-chart {
  grid-area: cases-by-chart;
}
